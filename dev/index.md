...
[`lm()`](https://rdrr.io/r/stats/lm.html)/[`glm()`](https://rdrr.io/r/stats/glm.html)/`brms`.
Visit [the mcp website](https://lindeloev.github.io/mcp/) for worked
examples and tips for many of them.
...
![](reference/figures/README-demo-plot_pars-1.png)
...
![](reference/figures/README-demo-pp_check-1.png)
...
- `mcp` currently supports specific combinations of families
  ...
  and link functions (`identity`, `logit`, `probit`, and `log`).
...
- `~ 0 + sigma(1)` models an intercept change in standard deviation.
  ...
  Explicit [`sigma()`](https://rdrr.io/r/stats/sigma.html) formulas use
  a log link, so their coefficients are on the log-SD scale.
