...
-   **Missing response imputation:** Missing responses are now retained as posterior JAGS imputations. `predict(..., newdata = fit$data[is.na(fit$data$y), ])` can be used to see imputed values - similarly for `fitted()`. It supports AR/MA too. Missing values in AR/MA are not supported in `log_lik()`, `loo()`, and `waic()` who will throw an informative error.
