 ...
 * Many updates to `plot()`.
   - Includes the posterior densities of the change point(s). Disaple using `plot(fit, cp_dens = FALSE)`.
   ...
   - Plot prediction intervals using `plot(fit, q_predict = TRUE)`. Can now be added on top of `q_fit`.
   - Choose data geom, e.g., using `plot(fit, geom_data = "line")` (useful for time series) and disable using `geom_data = FALSE`.
