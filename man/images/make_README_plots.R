...
save_it = function(filename) {
  ggplot2::ggsave(paste0("doc/", filename), width=6, height=3, dpi = 100)
}
...
data = tibble::tibble(x = 1:100,
              y = empty$func_y(x, 50, 20, 40, 20))
fit = mcp::mcp(segments, data, par_x = "x")
...
data = tibble::tibble(x = 1:100,
              y = empty$func_y(x, 20, 3, -2, 30, 15))
fit = mcp::mcp(segments, data)
...
data = tibble::tibble(id = c("John", "Benny", "Rose", "Cath", "Bill", "Erin")) %>%
  ...
  dplyr::mutate(
    ...
    y = empty$func_y(x, 15, 3, -2, 40, 25, 7*(id2 - mean(id2)))
  )
fit = mcp::mcp(segments, data)
...
fit = mcp::mcp(segments, data, family = binomial())
...
empty = mcp::mcp(segments, sample=FALSE)
data = tibble::tibble(x = 1:100,
              y = empty$func_y(x, 20, -3, 20, 5, 30, 45, 30))
fit = mcp::mcp(segments, data, prior, n.adapt = 2000, n.update = 2000)
