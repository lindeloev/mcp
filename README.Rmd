...
[Group-level change points](https://lindeloev.github.io/mcp/articles/group_effects.html):

- Model group-level change points using `+(condition|id)`. Get posteriors using `ranef(fit)`.
...
- Missing responses are sampled in JAGS and retained with their matching posterior draws. \* Use `fitted(fit) |> filter(is.na(y))` for expected responses and `predict(fit) |> filter(is.na(y))` for posterior imputations.
