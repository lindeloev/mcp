...
Use `summary()` to summarise the posterior distribution as well as sampling diagnostics. They were simulated using `mcp` (see `mcp_example("demo")$example_code`) so the summary include the "true" values in the column `sim` and the column `match` show whether this true value is within the interval:
...
`rhat` is the rank-normalized split-Rhat convergence diagnostic; `ess_bulk` and `ess_tail` are the effective sample sizes for the bulk and tails of the posterior. Warning thresholds can be changed with `mcp(..., diagnostics = list(rhat = 1.01, ess_bulk = 400, ess_tail = 400))`; `summary()` inherits these settings. You may also want to do a posterior predictive check using `pp_check(fit)`.
...
## Means
Find the single change point between two plateaus (simulated using `mcp_example("intercepts")$example_code`).
...
fit = suppressMessages(mcp_example("group_cp", plot = FALSE))
set.seed(42)
theme_it(plot(fit, facet_by = "id"), "Varying slope change")
```

Summarise the group-level change-point deviations using `ranef()` or plot them using `plot_pars(fit, "group")`. Again, these data were simulated using `mcp` (see `mcp_example("group_cp")$example_code`), so the `match` and `sim` columns show the simulation values and whether they are inside the interval. Set `width` wider for a more lenient criterion.
...
Here is a binomial change point model with three segments (see simulation code: `mcp_example("binomial")$example_code`). We plot the 95% central posterior interval too:
...
fit = mcp(model, data, warmup = 5000, iter = 5000)
