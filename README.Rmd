...
You can do change point regression for a large number of models in a syntax that aligns with `lm()`/`glm()`/`brms::brm()`. Visit [the mcp website](#0) for worked examples and tips for many of them.
...
For model comparisons, we can fit a null model and compare the predictive performance of the two models using (approximate) leave-one-out cross-validation ([see more here](https://lindeloev.github.io/mcp/articles/comparison.html)). Let's specify a null model without the first plateau - so it is just one straight line:
