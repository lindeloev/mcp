 ...
 * [Install the latest version of JAGS](https://sourceforge.net/projects/mcmc-jags/). Linux users can fetch binaries [here](http://mcmc-jags.sourceforge.net/).
 
 * Install `mcp` by running this in R: `devtools::install_github("lindeloev/mcp")`. If you don't have `devtools`, install it first using `install.packages("devtools")`.
...
Under the hood, order restriction is automatically applied to `cp_*` parameters if you don't do it yourself, so that they are in the correct order on the x-axis. The one exception is for `dunif` distributions where you have to do it as above. In addition to the model parameters, `MINX`, `MAXX`, `SDX`, `MINY`, `MAXY`, and `SDY` are also available. They are used to set uninformative default priors that scales with data. If you now ran `mcp(segments, prior, sample=FALSE)` you could see your priors replace the defaults.


Enough talk. Let's start sampling:
