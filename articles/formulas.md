...
2.  The “cp” part specifies change point modeling. It can be `1 ~ ...`
    ...
    (random) change-point
    deviations](https://lindeloev.github.io/mcp/articles/group_effects.md).
3.  The predictor part is regular `~ 1 + x` or
    ...
    more](https://lindeloev.github.io/mcp/articles/dpar.md)). The same
    applies to time-series such as `ar(1)` for intercept on first-order
    autogregression and `ma(2, 1 + x)` for second-order MA regression
    ([read more](https://lindeloev.github.io/mcp/articles/arma.md)).
...
Use
[`mcp_pars()`](https://lindeloev.github.io/mcp/reference/mcp_pars.md) to
inspect the parameters, including their roles, distributional
...
- `cp_i_group` contains the group-level deviations from population-level
  ...
  mcp](https://lindeloev.github.io/mcp/articles/group_effects.md).
...
- `sigma_*` describe the Gaussian residual standard deviation; [read
  more here](https://lindeloev.github.io/mcp/articles/dpar.md). With an
