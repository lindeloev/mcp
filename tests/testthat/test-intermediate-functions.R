...
test_that("interpolate_newdata() does not invent response auxiliaries", {
  ...
  expect_equal(interpolate_newdata(fit, x_values = 1:3, at = list(N = 4))$N, rep(4, 3))
  expect_error(interpolate_newdata(fit, at = list(N = 2.5)), "N.*integerish")
  expect_error(fitted(fit, newdata, probs = 0, rate = TRUE), "strictly between 0 and 1")
  expect_error(predict(fit, rate = TRUE, probs = 1), "strictly between 0 and 1")
  expect_true("Q50" %in% names(fitted(fit, newdata, probs = 0.5, rate = TRUE)))
  plot_none = plot(fit, lines = 0, q_fit = FALSE, cp_dens = FALSE)
  plot_median = plot(fit, lines = 0, q_fit = 0.5, cp_dens = FALSE)
  expect_gt(length(plot_median$layers), length(plot_none$layers))
  ...
  expect_s3_class(plot(fit, rate = FALSE, at = list(N = 4), cp_dens = FALSE), "ggplot")
  expect_error(
