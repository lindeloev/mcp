...
A unique feature of `mcp` is modeling change points with group-level effects (also called varying or random effects). This lets the change point vary by a grouping factor while other parameters remain population-level. The terminology follows `brms`: `fixef()` reports population-level effects and `ranef()` reports group-level deviations.
...
You specify group-level effects using the familiar [`lmer`](https://www.rdocumentation.org/packages/lme4/versions/1.1-21/topics/lmer) and `brms` syntax `(1|group)`. Currently `mcp` supports this syntax only in the cp part, and only for intercepts. For example, here we model a varying change point between a plateau and a joined slope:
...
varying effects in the cp part, or supply an exact name such as
`varying = "cp_1_id"`. The reserved selector `varying = "predictor"` is ready
for predictor-side group-level effects; because those terms are not yet
supported, it currently selects no effects. `ranef(fit)` deliberately remains
simple and returns all group-level effects.
