...
Because of the identity-link, the regression coefficients are interpretable as intercepts and slopes on `P(success)` in contrast to the "usual" log-odds fitted when `family = binomial(link = "logit")`. For example, `Intercept_1` is inferred probability of success at `x = 0` and likewise for `Intercept_2` at `x = cp_1`.
