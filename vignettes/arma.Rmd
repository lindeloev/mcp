...
You can do hypothesis testing (see `hypothesis()`) and model comparisons (see `loo()`) with GARMA models, just as with any other model in `mcp`. [Read more here](../articles/comparison.html) or scroll down for an applied example.
...
The full suite of [`mcp` formula syntax](../articles/formulas.html) is available inside both terms, so you could use `ar(3, 1 + I(x^2) + exp(x))` or `ma(1, 0 + group)`. Make sure to use a positive-only prior (e.g., `"dunif(0, 1)"`) with transformations such as `log()` and `sqrt()`, since they fail for negative values.
