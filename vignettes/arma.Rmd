...
Notice that `sigma` in AR models describe *innovations*, i.e., the part of the residuals that are not explained by the autoregressive coefficients. `sd(ex$data$price)` is always higher. In this case, the SD of raw data in the plateau is `r round(sd(ex$data$price[1:120]), 2)`. As always, it is good to assess posteriors and convergence more directly:
...
You have the full suite of [`mcp` formula syntax](../articles/formulas.html) available inside `ar` so you could easily do  `ar(3, 1 + I(x^2) + exp(x))`. Make sure to use a positive-only prior (e.g., `"dunif(0, 1)"` if using `log()` and `sqrt()` since they fail for negative values.
...
You could use `plot(fit, geom_data = "line")` for a more classical line plot of the time series data. You can omit plotting AR effect entirely setting `plot(fit, arma = FALSE)`. You can also plot the `ar1_` parameters directly using `plot_dpar()`, so the y-axis is the value of `ar1`:
