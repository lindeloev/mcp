...
Let's plot it (<span style="color:red">OBS: updated in `mcp` v0.3</span>) and we see that AR was strong in the first segment and weaker-but-negative in the second:
...
Note that he naming syntax for autoregressive intercepts is `ar[order]_[segment]`. For example, `ar1_2` is the first-order autoregressive coefficient in segment 2. For slopes it will be `ar[order]_[normal mcp name]`, e.g., `ar1_x_3` for a slope on AR(1) in segment 3. In the present dataset, the AR coefficients were simulated from `ar1_1 = 0.7`, `ar2_1 = 0.2`, and `ar1_2 = -0.4`, and these values are reasonably recovered.

In fact, the posterior mean is almost always exactly the same as `arima(data, order = c(N, 0, 0))` (see below) , so the non-perfect fits are due to randomness in the simulation - not in the fit.

Notice that `sigma` in AR models describe *innovations*, i.e., the part of the residuals that are not explained by the autoregressive coefficients. `sd(ex_ar$price)` is always higher. In this case, it is `r sd(ex_ar$price)`. As always, it is good to assess posteriors and convergence more directly:
...
```{r}
# Model
segments = list(response ~ 1 + ar(3))

# Simulate data
empty = mcp(segments, sample = FALSE, par_x = "time")
...
# Base arima AR(3)
arima(data$response, order = c(3, 0, 0))
```
...
```{r, cache = TRUE, results = FALSE}
fit = mcp(segments, data, par_x = "time")
```
...
Let's simulate a change point in autocorrelation and see if we can infer it later (<span style="color:red">OBS: this is new in `mcp` v0.3</span>):
...
... and we use a prior to equate the slopes of each segment (read more about [using priors](../articles/priors.html) to equate parameters and define constants). Now let's see if we can recover these parameters.
...
As usual, we can test hypotheses and do model comparison ([read more here](../articles/comparison.html)). For example, what is the evidence that `ar1_1` is more than 0.3 greater than `ar1_2`? Answer: It is quite compelling.

```{r}
hypothesis(fit, "ar1_1 - 0.3 > ar1_2")
```

Or how does this compare to a model with just the slope and no autocorrelation? Answer: quite strong with an `elpd_diff/se_diff` factor of around 7.
