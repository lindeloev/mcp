...
As explained in [../docs/articles/formulas.html], fixed change points can be implemented in almost all regression functions. You simply have an indicator variable saying whether there has been as shift at the corresponding x-value and use it like `lm(y ~ 1 + x * I(x > 30), data = df)`. Or if you don't want a carry-over effect from the first segment, do `lm(y ~ 1 * I(x < 30) + x * I(x >= 30), data = df)`. This will work for most regression packages in R, including `survival` and `lme4`.
