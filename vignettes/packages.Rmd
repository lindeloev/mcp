 ...
 * *You have a lot of data and limited time*: While `mcp` is reasonably fast for typical problems, MCMC sampling is slower than analytical and specialized solutions. For datasets > 20.000 points and a complicated model, it may take `mcp` hours (but not days) to fit. See [tips and tricks](../articles/tips.html) how to speed up `mcp`. Briefly, configure a parallel future plan and lower the number of iterations: `future::plan(future::multisession, workers = 4)` followed by `mcp(..., iter = 1000, adapt = 400)`.
