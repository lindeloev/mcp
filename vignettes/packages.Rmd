 ...
 * *Automatic detection of change points*: most packages does automatic change point detection. This is not a space that `mcp` tries to fill so use the other packages here. But beware that many of the automatic procedures did not capture a change point in the worked examples below.
...
```{r}
library(strucchange)
fit_bp = breakpoints(y ~ 1, data = data, breaks = 2)
summary(fit_bp)
```
