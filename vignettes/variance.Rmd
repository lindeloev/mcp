---
title: "Modeling standard deviation and its changes"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Modeling standard deviation and its changes}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

For Gaussian models, you can explicitly model the residual standard deviation (`sigma`). For example, a plateau mean with an increasing standard deviation can be written as `y ~ 1 + sigma(1 + x)`. An explicit `sigma()` formula uses a log link, so its linear predictor is log-SD and `sigma = exp(eta_sigma)`, as in `brms`. This guarantees a positive, smoothly varying standard deviation. Formula syntax allowed outside `sigma()` (where it applies to the mean) also works inside it (where it applies to log-SD). For example, `~ 1 + sigma(1 + z + I(x^2))` is valid. Read more about mcp formulas [here](../articles/formulas.html).
...
Without an explicit `sigma()` formula, the default is the intercept-only prior
`dt(0, max(2.5, round(mad(y), 1)), 3) T(0, )` directly on the residual SD. The
response-calibrated scale adapts to the units of `y`, with a minimum of 2.5 to
avoid an accidentally narrow prior. This matches `brms`.

With an explicit `sigma()` formula, the intercept instead defaults to
`dt(0, 2.5, 3)` on log-SD, with scaled Student-t coefficient priors. `brms`
uses the same log-SD parameterization and intercept prior but flat
coefficients; the proper `mcp` defaults support prior sampling and mildly
regularize short segments.
