...
```{r, collapse=TRUE, R.options = list(width = 9999)}
...
The default [uniform order statistics prior on change points](../articles/priors.html) has flat symmetric properties across the range, but an informed prior should be used for point-null hypothesis testing.
 ...
 * The only constraint is your imagination. How about all hypotheses we may have at once? `cp_1 > 3.5 & cp_1 < 4.5 & items_2 < -0.4 & Intercept_1 > 2.5` against its inverse `(cp_1 < 3.5 | cp_1 > 4.5) & items_2 > -0.4 & Intercept_1 < 2.5`
...
There are more examples [in the documentation for `hypothesis`](../reference/hypothesis.html), including how to test group-level deviations and [relative changes between segments](../articles/formulas.html#relative-changes-between-segments).
`mcp` evaluates the directional hypothesis for both posterior and prior samples. The returned `prob` is the posterior probability by default, or the prior probability with `prior = TRUE`; the Bayes factor is the posterior odds divided by the prior odds:
...
The strength of LOO-CV is that you can compare any N models, as long as they are models of the same data. In general, LOO-CV is the only inferential method for non-nested models in `mcp`. For example:

 * The existence of one or several change points. The [article on Poisson change points](../articles/poisson.html) contains an example of comparing a change-point model to a model without change points.
...
You can read more about Leave-One-Out Cross Validation elsewhere, but briefly, it does this:
 ...
 2. Computes the posterior density at the left-out data point (out-of-sample data), i.e., the height of the posterior at that data point. For example, if the posterior is a normal distribution, a data point near the mean of the posterior has higher density (it is less "surprising") than if it is at $z = -3$. Better predictions mean higher densities, i.e., less surprisal.
...
This means that once model2 (`fit_info`) has done its "predicting", the others add very little over and above that. If you want to learn about how well they predict relative to each other, use `loo::pseudobma_weights()` which weights proportional to the ELPD of each model.
