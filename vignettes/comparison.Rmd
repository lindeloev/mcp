...
# A very simple model
segments_simple = list(recalled | trials(items) ~ 1 + items)
```

Notice that `items` is used both as `trials` and x-axis. No problem. 


Simulate some data using `fit$func_y`:

```{r}
library(mcp)
empty = mcp(segments, family = binomial(), sample = FALSE)

set.seed(42)
data = tibble::tibble(
  items = rep(1:9, each = 40),
  recalled = empty$func_y(items, int_1 = 2.5, items_2 = -0.4, cp_1 = 4, sigma = 0.5)
)
```


Now fit it with and without the informative prior. We set `iter` fairly high because this model is not sampled effectively. Running in parallel (`cores = 3`) speeds it up. We will also add `sample = "both"` to sample *both* prior and posterior. We will need both to compute point Bayes Factors using `hypothesis()` later.

```{r, cache = TRUE, message = FALSE, warning = FALSE}
fit_default = mcp(segments, data, family = binomial(), sample = "both", iter = 10000, cores = 3)
fit_info = mcp(segments, data, prior, family = binomial(), sample = "both", iter = 10000, cores = 3)
fit_simple = mcp(segments_simple, data, family = binomial(), sample = "both", iter = 10000, cores = 3)
```
 ...
 * Only constaint is imagination. How about all hypotheses we may have at once? $cp_1 > 3.5 & cp_1 < 4.5 & items_2 < -0.4 & int_1 > 2.5$ against it's inverse $(cp_1 < 3.5 | cp_1 > 4.5) & items_2 > -0.4 & int_1 < 2.5$
...
And as with Bayes Factors, you can obtain positive evidence for null models. The reason simpler models can be preferred in Bayes is that each new parameter increases the prior predictive space, and thereby comes with a greater "risk" of making way-off predictions. If the parameter does too little to "make up" for this by increasing the likelihood, it is a net negative for predictive accuracy, and the simpler model will be preferred. So *narrower priors are simpler models*.

Also, if the data is reasonably informative (it almost always is), LOO-CV is much less influenced by priors. For LOO, the priors can more be thought of as a regularization (a way to ensure that sampling is efficient) than as the point of departure which all our inferences are relative to (Bayes Factors).
...
What is interesting is the *relative* differences in these predicted densities. We can compare the models using `loo::loo_compare()`.The `mcpfit` objects have placeholders at `fit$loo` and `fit$waic`. You need not use them, but it's nice to keep things together, especially if you want to `save(fit)` the objects for later use.
