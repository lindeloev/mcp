 ...
 * `post_prob`: This is `NA` because equality is not an event with positive probability in the continuous model.
...
There are more examples [in the documentation for `hypothesis`](../reference/hypothesis.html), including how to test group-level deviations and [relative changes between segments](../articles/formulas.html#relative-changes-and-differences-between-segments).
`mcp` evaluates the directional hypothesis for both posterior and prior samples. The posterior probability is reported as `post_prob`, and the Bayes factor is the posterior odds divided by the prior odds:
