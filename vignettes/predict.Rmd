...
## Arguments
If you look at the documentation for `predict()`, `fitted()`, and `residuals()`, you'll see that they are quite versatile, taking many different arguments. To mention a few, you can set  `fitted(fit, dpar = "sigma")` to get fitted values for `sigma` [more on modeling sigma](../articles/variance.html), `prior = TRUE` to predict using only the prior, `varying = TRUE` and `arma = FALSE` can be toggled to include/exclude AR(N) and varying effects.
