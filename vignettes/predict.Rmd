...
## Arguments
If you look at the documentation for `predict()`, `fitted()`, and `residuals()`, you'll see that they are quite versatile, taking many different arguments. To mention a few, you can set  `fitted(fit, dpar = "sigma")` to get fitted values for `sigma` [more on modeling sigma](../articles/variance.html), `prior = TRUE` to predict using only the prior, and use `varying = TRUE` and `arma = FALSE` to include/exclude varying and AR/MA effects.
