...
`summary` uses 95% central posterior intervals by default, but you can change it using `summary(fit, width = 0.80)`. If you have [varying effects](../articles/varying.html), use `ranef(fit)` to see them.
...
The priors on change points are discussed extensively in the prior vignette. With the default logit link, intercepts and categorical contrasts use `dt(0, 2.5, 3)`: a weakly regularizing Student-t prior inspired by `brms`. This covers extreme probabilities without being effectively flat. The figure compares Student-t priors with scales 1 (red), 2.5 (black, the `mcp` default), and 5 (blue), together with their correspondence to probabilities:
...
ggplot(data.frame(logits = 0), aes(x = logits)) + 
  ...
  stat_function(fun=scaled_t, args = list(scale = 2.5), lwd=2, col="black") +
  stat_function(fun=scaled_t, args = list(scale = 5), lwd=2, col="blue") +
...
Numeric coefficient scales are divided by a representative change in their model-matrix column: its range when it has two values, and two standard deviations otherwise. Terms involving local `par_x` additionally use the expected segment width, `(max(x) - min(x)) / n_segments()`. The implied change across a typical segment therefore has the same 2.5-logit scale, even when the model contains several change points.
