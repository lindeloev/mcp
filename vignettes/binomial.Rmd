...
`summary` uses 95% central posterior intervals by default, but you can change it using `summary(fit, width = 0.80)`. If you have [group-level effects](../articles/group_effects.html), use `ranef(fit)` to see their deviations.
...
These plots work with [group-level effects](../articles/group_effects.html) as well.
...
`mcp` uses priors to achieve a lot of its functionality. See [how to set priors](../articles/priors.html), including how to share parameters between segments and how to fix values. Here, I post a few notes about the binomial-specific default priors.
...
```{r, results='hold', R.options = list(width = 9999)}
