...
`empty$func_y` is now a function that can predict data given these parameters. If you are in a reasonable R editor, type `empty$func_y(` and press TAB to see the required arguments. I came up with some values below, including change points at $year = 25$ and $year = 65$. Notice that because `binomial()` defaults to the link function `link = "logit"`, the intercept and slopes are on a [logit scale](https://en.wikipedia.org/wiki/Logit). Briefly, this extends the narrow range of binomial rates (0-1) to an infinite logit scale from minus infinity to plus infinity. This will be important later when we set priors.
...
fit = mcp(segments, data, family = binomial())
```
...
`summary` uses 95% highest density intervals (HDI) by default, but you can change it using `summary(fit, width = 0.80)`. If you have [varying effects](../docs/articles/varying.html), use `ranef(fit)` to see them.
...
These lines are just `fit$func_y` applied to a random draw of the posterior samples. In other words, they represent the joint distribution of the parameters. You can change the number of draws (lines) using `plot(fit, lines = 50)`.
...
Of course, these plots work with [varying effects](../docs/articles/varying.html) as well.
...
`mcp` uses priors to achieve a lot of it's functionality. See [how to set priors](../docs/articles/priors.html), including how to share parameters between segments and how to fix values. Here, I post a few notes about the binomial-specific default priors.
