---
title: "Poisson change point analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Poisson change point analysis}
...
Default priors were used. The intercept prior is a Student-t distribution centered on the rounded median of `log(pmax(n, 0.1))`, with scale equal to the maximum of 2.5 and its rounded MAD. This follows the robust calibration used by `brms` while handling zero counts. Categorical contrasts use scale 2.5. Numeric coefficient scales are divided by their binary range or two standard deviations, with terms involving local `par_x` additionally divided by the expected segment width:
