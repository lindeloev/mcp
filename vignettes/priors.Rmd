...
Now, let's simulate some data that from the model specified by `segments`. The following priors are "at odds" with the actual data so as to show their effect.
...
fit_manual = mcp(segments, data, prior, sample = "both", adapt = 10000)
fit_default = mcp(segments, data, sample = "both", adapt = 10000)
```
...
fit_two = mcp(segments_two, data_dummy, sample = "prior", chains = 1, iter = 300000)

# Sample priors for five default change points
segments_five = list(y ~ x, ~ 1, ~1, ~1, ~1, ~1)
fit_five = mcp(segments_five, data_dummy, sample = "prior", chains = 1, iter = 300000)
```

```{r, echo = FALSE, fig.height=6, fig.width=7}
par(mfrow = c(2,2))
MINX = min(data_dummy$x)
MAXX = max(data_dummy$x)
...
plot(y1$x, y1$y + y2$y, type = "l", lty = 2, xlab = NA, xlim = c(MINX, MAXX), main = "t-tail two", ylab = "Density")
...
plot(z5, xlab = NA, xlim = c(MINX, MAXX), main = "t-tail five"); text(135, 0.08, labels = "cp_5")

lines(z1$x, (z1$y + z2$y + z3$y + z4$y + z5$y), lty = 2); text(100, 0.045, labels = "sum")
lines(z4); text(125, 0.025, labels = "cp_4")
lines(z3); text(100, 0.02, labels = "cp_3")
lines(z3); text(75, 0.025, labels = "cp_2")
lines(z2); text(55, 0.043, labels = "cp_1")
lines(z1)
...
*OBS: These priors are likely to change in versions beyond mcp 0.2, but not drastically.*
...
For 2+ change points, the default is `cp_i = dt(MINX, (MAXX-MINX) / N_CP, 1) T(cp_i-1, MAXX)`. This is not as complicated as it looks, so let me unpack it.

 * It is t-distributed with one degree of freedom (`dt(..., 1)`).
