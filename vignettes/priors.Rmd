...
Continuous slopes use the corresponding coefficient scale divided by the expected segment width, `(max(x) - min(x)) / n_segments()`. Thus the prior concerns the amount of change across a typical segment rather than depending arbitrarily on the units or number of change points. Categorical contrasts use the same scale without the width adjustment.
...
An explicit `sigma()` formula instead models log-SD, as in `brms`. Its intercept prior is `dt(0, 2.5, 3)`, and its proper contrast and slope priors use the same link-scale width, with continuous slopes divided by the expected segment width. These proper coefficient priors intentionally differ from the improper flat defaults in `brms` so that prior-only sampling remains defined.
