...
* Default population-level change-point priors are ordered. For user priors, `mcp` adds truncation (e.g., `T(cp_1, )`) only when the prior has neither explicit truncation nor an inherently bounded form such as `dunif()` or `dirichlet()`. User-specified bounds are respected as written, and group-level change-point deviations are not guaranteed to preserve the population-level ordering.
...
The default priors order the population-level change points from left (`cp_1`) to right (`cp_2+`). User priors can relax this ordering, and realized group-level change points are not necessarily ordered. `mcp` offers two built-in prior families with different tradeoffs:
...
* **Symmetric segment spacings:** Use the Dirichlet prior when its explicit joint prior on segment spacings matches the scientific question, particularly for Bayes factors or many change points. With equal concentration parameters it treats the spacings exchangeably; this is a substantive prior, not an uninformative one. It has well-known mathematical properties and precedent in [Bürkner & Charpentier (2019)](https://psyarxiv.com/9qkhj/). It is not the default because it can sample orders of magnitude less efficiently. In these cases, increase the number of MCMC draws (e.g., `mcp(..., iter = 20000)`).
 ...
 * It is t-distributed with $N - 1$ degree of freedom (`n_cp() - 1`). This ensures narrower priors as the number of change points increase, so as to avoid excessive accumulation of densities at high $x$.
 ...
 * The standard deviation is the distance between equally-spaced change points: `(max(x) - min(x)) / n_cp()`.
...
Group-level change-point deviations have additional constraints: they are exactly zero-centered, and their default priors bound each resulting change point relative to the adjacent population-level change points or observed range. These bounds do not guarantee that realized group-level change points are mutually ordered. Predictor-side deviations are mean-zero in their hierarchical distribution but are not forced to sum exactly to zero. See [group-level effects with mcp](https://lindeloev.github.io/mcp/articles/varying.html).
