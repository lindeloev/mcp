...
The default priors order the population-level change points from left (`cp_1`) to right (`cp_2+`). User priors can relax the population-level ordering. Models with group-level change points additionally constrain their realized locations to remain ordered within every group. `mcp` offers two built-in population-level prior families with different tradeoffs:
...
Group-level change-point deviations are mean-zero in their hierarchical distribution but are not forced to sum exactly to zero. Their default priors use sequential bounds, and the model constrains the resulting locations to remain ordered within each group. All group-level change points in one model must use the same grouping factor. Predictor-side deviations are also hierarchically mean-zero but have no ordering constraint. See [group-level effects with mcp](https://lindeloev.github.io/mcp/articles/varying.html).
