...
# DEFAULT TWO #
###############
draws_two = posterior::as_draws_df(fit_two)
...
# DEFAULT FIVE #
################
draws_five = posterior::as_draws_df(fit_five)
...
The first change point defaults to the rule `cp_1 = uniform(min = min(x), max = max(x))`. In other words, the change point has to happen in the observed range of x, but it is equally probable across this range. This is identical to the Dirichlet prior.
 ...
 * It is truncated to be greater than the previous `cp`. For example, `cp_3` is bounded below by `cp_2` and above by `max(x)`. Since `cp_0 = min(x)`, all change points are "forced" to be in the observed range of `x`.
