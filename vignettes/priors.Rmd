...
For Gaussian models, residual-SD priors are always calibrated from `mad(y)` on the response scale. This remains true for `gaussian(link = "log")`: the link constrains the conditional mean, not the observations, so non-positive responses are valid and are never passed through `log()` merely to construct a sigma prior.
