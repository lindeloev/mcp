...
plot.mcpfit = function(x, type="overlay", draws=25, pars="population", facet_by = NULL, ...) {
  ...
  if (!is.null(pars)) {
    if (pars != "population" & !all(pars %in% c(fit$pars$population, fit$pars$varying)))
  ...
  if (type == "combo") {
    ...
    if (length(pars) > 1) {
      ...
      return(bayesplot::mcmc_combo(fit$samples, pars = pars))
    } else {
      return(bayesplot::mcmc_combo(fit$samples, regex_pars = pars))
    }
  }
}
