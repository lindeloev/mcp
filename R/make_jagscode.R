...
#' @examples
#' 
make_jagscode = function(data, segments, prior) {
  ...
  par_name_y = as.character(segments[[1]][[2]])
  ...
  if(!length(all_slopes)) {
    if(!is.null(x)) par_name_x = x
    else stop("This is a plateau-only model, but `x` is missing: mcp(..., x = \"name\")")
  } else par_name_x = all_slopes[1]
  ...
  # Check that a change point can be detected
  if(nsegments < 2) stop("At least two segments are needed")
  ...
  data {
    # X values
    MINX = ", min(data[, par_name_x]), "
    MAXX = ", max(data[, par_name_x]), "
    MEANX = ", mean(unlist(data[, par_name_x])), "
    SDX = ", sd(unlist(data[, par_name_x])), "
    
    # Y values
    MINY = ", min(data[, par_name_y]), "
    MAXY = ", max(data[, par_name_y]), "
    MEANY = ", mean(unlist(data[, par_name_y])), "
    SDY = ", sd(unlist(data[, par_name_y])), "
  }
  ...
  for(i in 1:(nsegments - 1)) {
    par_name = paste0("cp_", i)
    all_pars[par_name] = par_name
    
    # If prior is manually specified
    if(par_name %in% names(prior)) {
      if(i > 1) trunc = paste0(" T(cp_", i-1, ", )")
      else trunc = ""
      par_value = paste0(prior[[par_name]], trunc)
    }
    ...
    # Add new line to mm
    new_prior = paste0("  ", par_name, " ~ ", par_value, "\n")
    mm_add(new_prior)
  }
  ...
  for(i in 1:nsegments) {
    ...
    if(attr(terms(segments[[i]]), "intercept")) {
      par_name = paste0("int_", i)
      all_pars[[paste0("segment_", i)]]["int_free"] = par_name
      
      # If the prior is manually specified
      if(par_name %in% names(prior)) {
        par_value = prior[[par_name]]
      }
  ...
  for(i in 1:nsegments) {
    ...
    if(length(attr(terms(segments[[i]]), "term.labels"))) {
      ...
      # Set parameter name
      par_name = paste0(par_name_x, "_", i)
      all_pars[[paste0("segment_", i)]]["slope_free"] = par_name
  ...
  for(i in 1:nsegments) {
    ...
    if(!is.null(segment_pars)) {  # If there are parameters for this segment
      ...
      if(has_slope) {
        mm_add(par_name_x, "_", i, " * (min(", par_name_x, "[i_], cp_", i, ") ")
  ...
  mm_add("\n\n    # Likelihood
      ", par_name_y, "[i_] ~ dnorm(y_[i_], 1 / sigma^2)")
  ...
  return(list(
    model = mm,
    all_pars = all_pars,
    par_name_x = par_name_x,
    par_name_y = par_name_y
  ))
}
